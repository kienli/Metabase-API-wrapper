...
class MetabaseAPI:
    ...
    def __init__(self, domain, email, password=None, basic_auth=False, is_admin=True):
        ...
        if not self.is_admin:
            print(''' If you are not Admin, you won't be able to copy, 
    ...
    def get_item_info(self, item_type,
        ...
        Use 'params' for providing arguments. E.g. to include tables in the result for databases,
        use: params={'include':'tables'}
        """
        ...
        if not item_id:
            ...
            item_id = self.get_item_id(item_type, item_name, collection_id=collection_id,
    ...
    def get_item_id(self, item_type, item_name, collection_id=None, collection_name=None, db_id=None, db_name=None,
                    table_id=None):
        ...
        if item_type in ['card', 'dashboard', 'pulse']:
            if not collection_id:
                if not collection_name:
                    ...
                    collection_id = self.get_item_id('collection',
                                                     collection_name) if collection_name != 'root' else None
                    item_ids = [i['id'] for i in self.get("/api/{}/".format(item_type)) if i['name'] == item_name
        ...
        if item_type == 'database':
            ...
            if type(res) == dict:  # in Metabase ver *.40.0 the format of the returned result for this endpoint changed
                res = res['data']
    ...
    def get_db_id(self, db_name):
        import warnings
        warnings.warn("The function get_db_id will be removed in the next version. Use get_item_id function instead.",
                      DeprecationWarning)
    ...
    def get_collection_id_from_dashboard_id(self, dashboard_id):
        try:
            dashboard_info = self.get('/api/dashboard/{}'.format(dashboard_id))
            collection_id = dashboard_info['collection_id']
        except Exception as e:
            raise ValueError('There is no dashboard with this dashboard_id "{}"'.format(dashboard_id))
    ...
    def get_columns_name_id(self, table_name=None, table_id=None, column_id_name=False):
        """
        Return a dictionary with col_name key and col_id value, for the given table_id/table_name
        in the given db_id/db_name.
        ...
        if not table_name:
            ...
            if self.friendly_names_is_disabled():
                table_name = self.get_item_name(item_type='table', item_id=table_id)
            else:
                all_db_tables = {}

                for table in self.get("/api/database/{}/schema/public".format(db_id)):
                    all_db_tables.setdefault(table['id'], {})
                    all_db_tables[table['id']]['name'] = table['name']
                    all_db_tables[table['id']]['display_name'] = table['display_name']

                table_name = all_db_tables[table_id]['display_name']
    ...
    def friendly_names_is_disabled(self):
        """
        The endpoint /api/database/:db-id/fields which is used in the function get_columns_name_id relies
        on the display name of fields.
        If "Friendly Table and Field Names" (in Admin Panel > Settings > General) is not disabled,
        it changes the display name of fields.
    ...
    def create_card(self, card_name=None, collection_name=None, collection_id=None,
        ...
        if not card_name and (not custom_json or not custom_json.get('name')):
            raise ValueError(
                "A name must be provided for the card "
                "(either as card_name argument or as part of the custom_json ('name' key)).")
        if not table_id:
            if not table_name:
                raise ValueError('Either the name or id of the table must be provided.')
    ...
    def create_collection(self, collection_name, parent_collection_id=None, parent_collection_name=None,
        ...
        parent_collection_name -- name of the collection where the created collection resides in
        ...
        res = self.post('/api/collection',
                        json={'name': collection_name, 'parent_id': parent_collection_id, 'color': '#509EE3'})
        if return_results:
            return res
    ...
    def create_segment(self, segment_name, column_name, column_values, segment_description='',
        ...
        return_segment --    whether to return the created segment info (default False)
        """
        ...
        columns_name_id_mapping = self.get_columns_name_id(table_name=table_name)
        column_id = columns_name_id_mapping[column_name]
    ...
    def copy_collection(self, source_collection_name=None, source_collection_id=None,
        ...
        if type(items) == dict:  # in Metabase v. *.40.0 the format of the returned result for this endpoint changed
            items = items['data']
    ...
    def get_card_data(self, card_name=None, card_id=None, collection_name=None,
        ...
        The data_format keyword specifies the format of the returned data:
            ...
            - 'csv': the entire result is returned as a string, where rows are separated by newlines
                                                                                                and cells with commas.
        To pass the filter values use 'parameters' param:
            The format is like [{"type":"category","value":["val1","val2"],
            "target":["dimension",["template-tag","filter_variable_name"]]}]
    ...
    def clone_card(self, card_id,
        ...
        In the current version of the function there are some limitations which would be removed in future versions:
            - The column names used in filters need to be the same in the source and target table
                                            (except the ones that are ignored by 'ignore_these_filters' param).
            - The source and target tables need to be in the same DB.

        Keyword arguments:
        card_id -- id of the card
        target_table_id -- The ID of table that the filters of the cloned card would be based on
        target_table_name -- The name of the table that the filters of the cloned card would be based on
        ...
        ignore_these_filters -- A list of variable names of filters. The source of these filters would not
        ...
        source_table_id = card_info['table_id']
        ...
        if card_info['dataset_query']['type'] == 'native':
            ...
            card_info['dataset_query']['native']['query'] = card_info['dataset_query']['native']['query'].replace(
            ...
            for filter_variable_name, data in filters_data.items():
                ...
                card_info['dataset_query']['native']['template-tags'][filter_variable_name]['dimension'][
                    1] = target_col_id
        ...
        # simple/custom questions
            ...
            for source_col_id in source_column_ids:
                ...
                query_data_str = query_data_str.replace("['field', {}, ".format(source_col_id),
        ...
        res = self.create_card(custom_json=new_card_json, verbose=True, return_card=True)

        # Return the id of the created card
        return res['id']

    def clone_dashboard(self, source_dashboard_name=None, source_dashboard_id=None,
                        destination_dashboard_name=None, destination_collection_name=None,
                        destination_collection_id=None, target_table_id=None,
                        target_table_name=None, postfix='', return_cloned_dashboard_id=False):
        """
        ...
        source_collection_id = self.get_collection_id_from_dashboard_id(source_dashboard_id)
        ...
        card_id_mapping = {}
        for card_id in source_dashboard_card_ids:
            dup_card_id = self.clone_card(card_id=card_id, target_table_id=target_table_id,
                                          new_card_collection_id=cards_collection_id)
            card_id_mapping[card_id] = dup_card_id

        # replace cards in the duplicated dashboard with duplicated cards

        print("Replacing cards in the duplicated dashboard...")
        ...
        for card in dup_dashboard['ordered_cards']:
            ...
            # Map old and new columns for dashboard parameters/filters
            card_info = self.get_item_info('card', card['card_id'])
            ...
            for item in card_json['parameter_mappings']:
                item['card_id'] = new_card_id
                column_id = item['target'][1][1]
                column_name = source_table_col_id_name_mapping[column_id]
                target_col_id = target_table_col_name_id_mapping[column_name]
                item['target'][1][1] = target_col_id
        ...
        print('Dashboard is cloned successfully')

        if return_cloned_dashboard_id:
            return dup_dashboard_id
    ...
    def delete_item(self, item_type, item_name=None, item_id=None,
                    collection_name=None, collection_id=None):
        """
